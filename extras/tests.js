...
test(function hide_passwords (done) {
    ...
    log('The client users is now:', client.get('users'))
    assert(!client.get('users').val[0].pass)
    assert(!client.get('users').val[1].pass)
    assert( client.get('users').val[0].name)
    assert(!client.get('users').val[1].name)
    done()
})
...
test(function proxies (done) {
    ...
    // state properly.  They should trigger set() when they modify the
...
test(function set_can_trigger_toget (done) {
    ...
    var triggered = 0
    bus(() => {
        log('Aight! Geting set_trigger_get')
